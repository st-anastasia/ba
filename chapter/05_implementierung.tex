...
Die Untermenüs (Abb. \ref{fig:gallery_large}, oben) werden mit Hilfe des \textit{md-toolbar} Elements umgesetzt. Werkzeuge, welche nicht auf die Leiste passen, lassen sich mit \textit{md-menu-content} in ein Kontextmenü verstecken, welches erst beim Klicken erscheint.
...
Die Fotogalerie ist ein Exempel für die Kommunikation über mehrere Controller und Wiederverwendbarkeit von Models. Die Einzelansicht des Fotos ist ebenfalls ein Art Galerie, da hier ein Sliden zum nächsten und vorherigen Foto möglich ist. Beide Ansichten - Gallerie- und Einzelansicht laden die Fotos in Chunks (siehe \ref{imp:pagination}). Dieses gemeinsame Verhalten für das Laden von Fotos wird an ein separates Model, die \textit{PhotosGallery} ausgelagert.

Das \textit{PhotosGallery} Objekt wird zwischen den \textit{PhotosController} und \textit{PhotoDetailsController} wiederverwendet. Es speichert den aktuell geladenen Chunk an Fotometadaten sowie das aktuell ausgewählte Foto. Die \textit{PhotosGallery} lädt einen entsprechenden Chunk an Fotos, wenn der Benutzer auf eine Galerieseite navigiert. Das Sliden zum nächsten Foto in der Einzelansicht geschieht innerhalb des geladenen Chunks. Will der Benutzer ein weiteres Foto betrachten als im Chunk verfügbar, geschieht das Nachladen über den selben Mechanismus des paginierten Ladens in der Fotogalerie.

 Der \textit{PhotosController} und der \textit{PhotoDetailsController} beobachten den Zustand des \textit{PhotosGallery} Objekts. Unabhängig davon, welcher der beiden Controller eine Zustandsänderung im \textit{PhotosGallery} Objekt ausgelöst hat, bleibt die Darstellung immer synchronisiert. Der Benutzer kann somit mehrere Fotobünde weit in der Einzelansicht sliden. Wenn er am Ende wieder in die Gallerieansicht zurückkehrt, wird das aktuelle Fotobündel mit letzten betrachteten Foto dargestellt.
